...
## 2024-10-24 - Avoid repeated O(N) URL parsing in ReadManager

**Learning:** `ReadManager.isRead` previously invoked `ArticleIdentity.reconcileLegacyId`, re-parsing URLs heavily during every `@MainActor` SwiftUI evaluation (e.g. `ArticleListView`).
**Action:** Caching these legacy ID canonicalizations in `NSCache<NSString, NSString>` within `ReadManager` turns frequent parsing allocations into fast O(1) lookups during UI updates.
